...
//--------------------------------------------------
void ashen::Buffer::Destroy(const Context& context) const
{
	vmaDestroyBuffer(context., m_Buffer, m_Memory);
}
...
//    Accessors & Mutators
//--------------------------------------------------
const VkBuffer& ashen::Buffer::GetHandle() const { return m_Buffer; }
const VmaAllocation& ashen::Buffer::GetMemoryHandle() const { return m_Memory; }
VkDeviceSize ashen::Buffer::Size() const { return m_Size; }

//--------------------------------------------------
//    Commands
//--------------------------------------------------
void ashen::Buffer::InsertBarrier(const CommandBuffer& cmd,
...
void ashen::Buffer::CopyToBuffer(const CommandBuffer& cmd, const Buffer& dst, VkDeviceSize size, VkDeviceSize srcOffset, VkDeviceSize dstOffset) const
{
...
void ashen::Buffer::CopyToImage(const CommandBuffer& cmd, const Image& dst, VkExtent3D extent, uint32_t mip, uint32_t baseLayer, uint32_t layerCount) const
{
...
//    Constructor & Destructor
//--------------------------------------------------
ashen::BufferAllocator::BufferAllocator()
{
	m_CreateInfo = {};
...
//    Allocator
//--------------------------------------------------
ashen::BufferAllocator& ashen::BufferAllocator::SetDebugName(const char* name)
{
	m_pName = name;
	return *this;
}
...
ashen::BufferAllocator& ashen::BufferAllocator::SetMemUsage(VmaMemoryUsage usage)
{
...
ashen::BufferAllocator& ashen::BufferAllocator::AddInitialData(const void* data, VkDeviceSize dstOffset, uint32_t size)
{
...
void ashen::BufferAllocator::Allocate(const Context& context, Buffer& buffer) const
{
